<?php
function truncate ($string, $max = 1000, $rep = '') 
	{
	// Truncates string data
...
function validate_password($password, $min_char = 6, $max_char = 20) 
	...
	$eregi = eregi_replace('([a-zA-Z0-9_]{'.$min_char.','.$max_char.'})', '', $checkpassword); 
 
	if(empty($eregi)) 
		{ 
			if ($checkpassword == $password)
			// Only accepted if no leasding or trailing spaces exist and no oddball chars
				{
					return true;
				} else {
					return false;
